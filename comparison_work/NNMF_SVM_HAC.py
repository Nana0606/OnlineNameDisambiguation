...
# label is in second last column and year information is in last column
...
# use sklearn for svm classification and then report the probability output of each test instance
...
def Classification(train_set_list, train_label_list, test_set_list, test_label_list):
	...
	linear_svm = svm.SVC(probability = True);
	linear_svm.fit(X_train, Y_train);
	prob_matrix = linear_svm.predict_proba(X_test);
...
# use the output probability from SVM as meta features, and then utilize hierarchical clustering for anomaly detection
...
if __name__ == '__main__':
	...
	# best K via parameter tuning process
	K_list = [40];	
	K = 5;
	train_set_list, train_label_list, test_set_list, test_label_list, num_cluster_test = MF(filename, K);
	num_true_label = num_cluster_test;
	result_file = open('result_test.txt', 'w');
	result_file.write('K' + '\t' + 'cluster_size' + '\t' + 'F1' + os.linesep);

	# tune the pre-defined cluster_size in Hierarchical Clustering and choose the one which gives best F1 score
	cluster_size_list = range(num_true_label - 10, num_true_label + 5);
	for k_ in range(0, len(K_list)):
		K = int(K_list[k_]);
		cluster_size_f1_dict = {};
		for i in range(0,len(cluster_size_list)):
			...
			for run in range(0,5):
				...
				sum_ = sum_ + aver_f1;
			cluster_size_f1_dict[cluster_size] = sum_;
		# output the best aver_f1 with the cluster_size
		max_key = max(cluster_size_f1_dict, key=cluster_size_f1_dict.get);
		max_aver_f1 = cluster_size_f1_dict[max_key];
		result_file.write(str(K) + '\t' + str(max_key) + '\t' + str(float(max_aver_f1)/5) + os.linesep);
